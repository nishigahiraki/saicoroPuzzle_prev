...
document.addEventListener('DOMContentLoaded', function(evt) {
	document.querySelector('iframe.rankingView').addEventListener('load', function(evt) {
		...
		try{
			...
			if( hiScore == undefined ) { hiScore = 100; }
			hiScoreView.textContent = String(hiScore).padStart(10,0);
		}
		catch(error) {
			console.log('catch iframe ::::',error);
			hiScore = 100;
			hiScoreView.textContent = String(hiScore).padStart(10,0);
		}
	});
...
function deleteDiceAtList( LIST, targetFACE ) {
	...
	for(const temp of LIST) {
		...
		chainDeleteTotalList[targetFACE-1] += 1;
		console.log(chainDeleteTotalList);
		//
	...
	if(chainIs) {
		...
		chainDeleteTotalList[targetFACE-1] = 0;
	}
	//
	// 点数加算
	usrScore += targetFACE * LIST.length * ( chainNumList[ targetFACE - 1 ] + 1 );
	//usrScore += targetFACE * chainDeleteTotalList[targetFACE-1] * ( chainNumList[ targetFACE - 1 ] + 1 );
	//
	// ゲーム状態表示 HTML
